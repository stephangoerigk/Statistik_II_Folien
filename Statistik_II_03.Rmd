---
title: "Einheit 3"
subtitle: "Einfache lineare Regression (1)"
...
*  Um solche Zusammenhänge zu messen, gibt es sogenannte Assoziationsmaße z.B. Kovarianz oder Korrelation.
...
- a ist also der Wert wo $X=0$ ist. Also hat jemand mit 0 auf der UV einen AV Wert von `r round(coef(model)[["(Intercept)"]], 0)`.
...
$$R^2=\frac{\text{erklärte Varianz}}{\text{gesamte Streuung}}=\frac{\sum\limits _{i=1}^{n}(\hat{y}_i-\bar{y})^2}{\sum\limits _{i=1}^{n}(y_i-\bar{y})^2}=(\frac{s_{XY}}{s_X \cdot s_Y})^2$$
<br>
...
$$ R^2=(\frac{`r round(cov(sampledata$X, sampledata$Y), 2)`}{`r round(sd(sampledata$X), 2)`\cdot `r round(sd(sampledata$Y), 2)`})^2 = `r round((cov(sampledata$X, sampledata$Y) / (sd(sampledata$X) * sd(sampledata$Y)))^2, 2)`$$

Es können `r round((cov(sampledata$X, sampledata$Y) / (sd(sampledata$X) * sd(sampledata$Y)))^2, 2) * 100`% der Streuung um den Mittelwert von Y durch die Gerade erklärt werden.
...
* Das **Bestimmtheitsmaß** $(R^2)$ gibt an, wie viel Prozent (%) der Gesamtvarianz der AV durch die UV (also durch das Regressionsmodell) aufgeklärt werden.
]
