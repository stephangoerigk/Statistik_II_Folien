...
#### Struktur des Mixed Designs

.center[
```{r eval = TRUE, echo = F, out.width = "100%"}
knitr::include_graphics("bilder/mixed_design_schema.svg")
```
]
...
ggplot(df_long, aes(x = Time, y = Score, colour = Gruppe)) +
   stat_summary(size = 1.5) +
    ...
    labs(x = "Zeit", y = "AV") +
  scale_colour_manual(values = c("Gruppe1" = "#621C37",
                               "Gruppe2" = "#EE0071")) +
...
* aufgrund in der Zwischenzeit erfolgter Intervention (oder einfach aufgrund des Vergehens von Zeit)
...
Die Quadratsumme $(QS)$ entspricht der quadrierten Abweichung der Mittelwerte $(\bar{Zeit_{j}})$ vom Gesamtmittelwert $(\bar{G})$ multipliziert mit der Zellgröße $n$ und der Anzahl der Stufen der Gruppenvariable:
...
* Sphärizitätskorrektur (i.d.R. nicht apriori geplant bzw. man hofft auf Sphärizität)
]
...
* Es werden Variabilität Zwischen-Personen **(between-subject)** und Innerhalb-Personen **(within-subject)** unterschieden

* Zum Prüfen der Effekte müssen separate **Prüfvarianzen** berechnet werden.

* Als Voraussetzung wird die **Sphärizität** geprüft und ggf. korrigiert.

* Wenn Sphärizität erfüllt ist, können Post-Hoc Vergleiche mittels **Tukey-Test** geprüft werden.
]
]
