---
title: "Einheit 7"
subtitle: "Regressionsmodelle mit Interaktionen"
...
2. Interaktionseffekt
  ...
  * Wechselwirkung zwei UVs/Prädiktoren (Interaktion) auf die AV
...
$\rightarrow$ Im Modell berechnete Steigung $(b)$ repräsentiert durchschnittlichen Unterschied in AV, wenn Person nicht zu Referenzkategoie gehört
...
$$Erfolg = b_1 \cdot Erfahrung_{(hoch)} + b_2 \cdot Konfrontationsb. + \epsilon_i$$
...
$$Erfolg = b_1 \cdot Erfahrung_{(hoch)} + b_2 \cdot Konfrontationsb. +  b_3 \cdot (Erfahrung_{(hoch)} \cdot Konfrontationsb.)+ \epsilon_i$$
---
class: top, left
### Regressionsmodelle mit Interaktionen
...
* Bei Konstanthaltung von Konfrontationsbereitschaft ist Erfahrung signifikant mit Therapieerfolg assoziiert $(t(49) = 8.89, p < .001)$.

* Bei Konstanthaltung von Erfahrung ist Konfrontationsbereitschaft nicht signifikant mit Therapieerfolg assoziiert $(t(49) = 1.89, p = .066)$.

* Das Gesamtmodell kann 62% der Varianz des Therapieerfolgs erklären $(R^2 = .62)$.
]
...
**Achtung: Unterschiedliche Steigungsparameter für selben Prädiktor (Erfahrung) nach Hinzunahme weiterer Prädiktoren**
...
* Steigungsparameter unterscheiden sich marginal:
  ...
  * Modell ohne Konfrontationsbereitschaft: $b_{Erfahrung=hoch}=$ `r round(as.data.frame(summary(model1)$coefficients)[2,1], 2)`

  * Modell mit Konfrontationsbereitschaft $b_{Erfahrung=hoch}=$ `r round(as.data.frame(summary(model2)$coefficients)[2,1], 2)`
...
knitr::include_graphics("bilder/moderation_pfad.svg")
```
]
...
<small>
* Mit 1 Einheit zusätzlicher Konfrontationsbereitschaft nimmt der Therapieerfolg von Therapeut:innen mit niedriger Erfahrung um `r round(as.data.frame(summary(model)$coefficients)[3,1], 2)` Punkte ab $(t(48) = -4.14, p < .001)$.

* Mit 1 Einheit zusätzlicher Konfrontationsbereitschaft nimmt der Therapieerfolg von Therapeut:innen mit hoher Erfahrung um `r round(as.data.frame(summary(model)$coefficients)[4,1], 2)` Punkte mehr zu, als bei der Referenzgruppe $(t(48) = 6.51, p < .001)$.
...
* Der Unterschied zwischen den Steigungen der Gruppen mit niedriger und hoher Erfahrung ist signifikant $(b = 0.82, t(48) = 6.51, p < .001)$
]
...
**Anmerkung:**
<small>
* Output des Regressionsmodells enthält lediglich einen Signifikanztest für die Steigung der Referenzkategorie $(b_{Konfrontativ|Erfahrung=0} = -0.44, t(48) = -4.14, p < .001)$

* Steigung der anderen Gruppe lässt sich "per Hand" ausrechnen $(b_{Konfrontativ|Erfahrung=1} =$ `r round(as.data.frame(summary(model)$coefficients)[3,1], 2)` + `r round(as.data.frame(summary(model)$coefficients)[4,1], 2)` = `r round(as.data.frame(summary(model)$coefficients)[3,1], 2) +  round(as.data.frame(summary(model)$coefficients)[4,1], 2)`)

* Signifikanztest prüft jedoch nur, ob $b_{Konfrontativ|Erfahrung=0} \neq b_{Konfrontativ|Erfahrung=1}$ $(t(48)=6.51, p<.001)$, nicht ob $(b_{Erfahrung=1} \neq 0$
...
* Signifikanztest für die Steigung der Referenzkategorie (hohe Erfahrung = 0) nun möglich $(b_{Konfrontativ|Erfahrung=0} = 0.38, t(48) = 5.63, p < .001)$
]
...
Die einzelnen Steigungen und deren KIs lassen sich jedoch auch automatisch anzeigen:
...
Für jeden Modellfaktor stellt sich die Frage: "*Erklärt das Modell mit diesem Faktor signifikant mehr Varianz als ohne den Faktor?*"
...
<small>
* Mit 1 Einheit zusätzlicher Empathiefähigkeit (Empathie=hoch) nimmt der Therapieerfolg von Therapeut:innen mit niedriger Erfahrung um `r round(as.data.frame(summary(model)$coefficients)[3,1], 2)` Punkte zu $(t(48) = 2.06, p = .045)$.

* Mit 1 Einheit zusätzlicher Empathiefähigkeit (Empathie=hoch)  nimmt der Therapieerfolg von Therapeut:innen mit hoher Erfahrung um `r round(as.data.frame(summary(model)$coefficients)[4,1], 2)` Punkte weniger zu, als bei der Referenzgruppe $(t(48) = -0.09, p = .927)$.
...
**Moderation:** *Beeinflusst die Erfahrung den Effekt des IQ auf den Therapieerfolg?* (Beispielserklärung: Erfahrenere Therapeut:innen können Intelligenz besser einsetzen / geringe Intelligenz kompensieren.)
]
...
**VORSICHT:** Effekt von Erfahrung in diesem Modell nicht mehr signifikant $(t(48) = 0.48, p = .631)$ - Wie kann dies sein?
...
.pull-right[
* Im Omnibustest ist der Haupteffekt von Erfahrung signifikant $(F(1,48) = 70.96, p < .001)$ 
...
$\rightarrow$ Die neue 0 ist als "durchschnittlich intelligent" interpretierbar
]
...
.pull-right[
Modell mit **zentriertem IQ** als Prädiktor:
...
* Therapeut:innen mit hoher Erfahrung (Erfahrung=1) haben bei einem durchschnittlichen IQ (IQ=0) einen geschätzten Therapieerfolg von `r round(as.data.frame(summary(model)$coefficients)[2,1], 2)` Punkten mehr, als die Referenzgruppe $\rightarrow$ Nun wieder signifikant $(t(48) = 8.24, p < .001)$
...
$$Erfolg = b_1 \cdot Erfahrung_{(mittel)} + b_2 \cdot Erfahrung_{(hoch)} + b_3 \cdot Konfrontationsb. ...$$
$$...+  b_4 \cdot (Erfahrung_{(mittel)} \cdot Konfrontationsb.) +  b_5 \cdot (Erfahrung_{(hoch)} \cdot Konfrontationsb.) + \epsilon_i$$
---
class: top, left
### Regressionsmodelle mit Interaktionen
...
* Alle 3 Steigungen werden simultan auf Signifikanz getestet
...
.pull-right[
* $b_{Konfrontativ|Erfahrung=hoch}$ unterscheidet sich signifikant von $(b_{Konfrontativ|Erfahrung=niedrig}$ $(p < .001)$ und $b_{Konfrontativ|Erfahrung=mittel}$ $(p = .012)$

* $b_{Konfrontativ|Erfahrung=niedrig}$ und $b_{Konfrontativ|Erfahrung=mittel}$ unterscheiden sich nicht signifikant $(p = .065)$
...
Wenn Sie in Ihrer Klausur den Output einer multiplen Regression mit Interaktion berichten sollen, könnte dies so aussehen: 

.content-box-gray[
Eine multiple Regression mit Interaktion mit den Prädiktoren Kindheitstrauma sowie Resilienz und dem Kriterium Depressivität wurde durchgeführt. 
...
.content-box-gray[
Eine multiple lineare Regression mit Interaktion wurde durchgeführt, um die Effekte von Schlafqualität und Therapiegruppe (Warteliste vs. Psychotherapie) auf Depressivität zu untersuchen. 
...
.content-box-gray[
Eine multiple lineare Regression mit Interaktion wurde durchgeführt, um die Effekte von Lernstrategien (A vs. B) und Motivation (niedrig vs. hoch) auf das Testergebnis zu untersuchen. 
