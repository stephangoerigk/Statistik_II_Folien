...
#### Berichten der Ergebnisse nach APA anhand unseres Beispiels
...
Vor der Analyse wurden die Voraussetzungen überprüft. Der Levene Test zeigte kein signifikantes Ergebnis ( $p$ = .75). Die Annahme der Varianzhomogenität ist also erfüllt. Auch die Normalverteilungsannahme ist erfüllt: Die Shapiro Wilk Tests für Medikament.1 ( $p$ = .96), Medikament.2 ( $p$ = .28) sowie Placebo ( $p$ = .93) wurden nicht signifikant. 

Deskriptiv zeigte die Gruppe Medikament.1 die größte Verbesserung ( $M$ = 11.59, $SD$ = 3,62) im Vergleich zu den Gruppen Medikament.2 ( $M$ = 10,98, $SD$ = 3.96) und Placebo ( $M$ = 7.14, $SD$ = 3.70).
...
Ein Post-Hoc Test mit Tukey Korrektur zeigte keinen signifikanten Unterschied ( $p$ = .70) zwischen Medikament.1 und Medikament.2, aber signifikante Unterschiede zwischen Medikament.1 und Placebo (4.45, $p$ < .001) und Medikament.2 und Placebo (3.85, $p$ < .001)
...
**Statistischer Bericht: (Verkürzte Version bei händischer Berechung in Ihrer Prüfung)**
...
$F$(df<sub>Zähler</sub>, df<sub>Nenner</sub>) = F-Wert, $p$ = Signifiknaz, $ω^2$ = Effektstärke

Wichtig ist außerdem, dass Sie genau angeben, **um welche Art von ANOVA** es sich handelt – also z.B. einfaktoriell oder mehrfaktoriell, mit oder ohne Messwiederholung.

Berichten Sie immer das, was die Daten tatsächlich hergeben. Wenn eine Effektstärke berechnet wurde, gehört sie in den Bericht Wenn Post-Hoc Tests durchgeführt wurden, sollten auch deren Ergebnisse mit angegeben werden.  

Dasselbe gilt für die Prüfung der Voraussetzungen: Wenn Sie in der Aufgabe Informationen zu Tests wie dem Levene-Test oder dem Shapiro-Wilk-Test sehen, dann berichten Sie diese. Wenn hingegen keine Testergebnisse angegeben sind, können (und sollen) Sie auch keine angeben.  

Bleiben Sie also flexibel und passen Sie Ihre Berichterstattung daran an, was Ihnen vorliegt.
