...
#### Anwendungsbeispiel händisch (kleiner Datensatz)
...
$x_{im(erwartet)}=$ Erwarteter Wert der Person $m$ in der Messwiederholung $i$ des Messwiederholungsfaktors $A$.
...
* Prüfung, ob sich die Messzeitpunkte signifikant unterscheiden
...
.pull-left[
#### Anwendungsbeispiel R (größerer Datensatz)
...
Der Faktor Zeit zeigte einen signifikanten Einfluss auf die Symptomschwere $F$(2, 38) = 116.08, $p$ < .001, $\eta^2_p$ = 0.86. Damit konnten 86 % der Varianz durch den Messwiederholungsfaktor aufgeklärt werden - dies entspricht einem starken Effekt. Der Mauchly-Test war nicht signifikant ($p$ = .670), was auf eine erfüllte Sphärizitätsannahme hinweist.
]
...
* Als zusätzliche Voraussetzung wird die **Sphärizität** geprüft.

* Bei Verletzungen der Sphärizitätsannahme können **Korrekturverfahren** angewendet werden, die Überschätzung des Effekts verhindern.
