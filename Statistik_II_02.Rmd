---
title: "Einheit 2"
subtitle: "Einfaktorielle Varianzanalyse (2)"
...
```{r echo=F}
df_f <- data.frame(Effektstärke = c("Kleiner Effekt", "Mittlerer Effekt", "Großer Effekt"),
                   f = c("0.10", "0.25", "0.40"))

names(df_f) = c("Effektstärke", "Cohen's f")
...
```{r echo=F}
df <- data.frame(Effektstärke = c("Kleiner Effekt", "Mittlerer Effekt", "Großer Effekt"),
                 EtaQuadrat = c("0.01", "0.06", "0.14"))

names(df) = c("Effektstärke", "Eta-Quadrat")
...
1. Varianz zwischen
  ...
  * Je größer $N$, desto größer die Varianz zwischen (steht im Zähler des F-Bruchs)
...
* Bei Annahme eines Signifikanzniveaus von $\alpha=0.05$, einer ANOVA mit 3 Gruppen und einer kleinen bis mittleren Effektstärke von $f=0.20$ wären für eine Teststärke $(1-\beta)$ von 90% lediglich $N=106$ Personen notwendig.
...
* Signifikantes Ergebnis führt zur Ablehnung der $H_0$ und zur vorläufigen Annahme der $H_1$
...
* Signifikante ANOVA: Es ist lediglich gesichert, dass die Gruppe mit dem kleinsten Mittelwert sich signifikant von der Gruppe mit dem größten Mittelwert unterscheidet.
...
* Kritische q-Werte stehen in einer eigenen Tabelle
...
Vor der Analyse wurden die Voraussetzungen überprüft. Die Annahme der Varianzhomogenität konnte nach Prüfung mit dem Levene Test bestätigt werden ( $p$ = .748). Auch die Normalverteilungsannahme, welche mit Shapiro Wilk Tests überprüft wurde, war gegeben (Medikament 1: $p$ = .962; Medikament 2: $p$ = .281; Placebo: $p$ = .928). 
